...
void modelStatesCallback(const gazebo_msgs_new::ModelStatesConstPtr &msg)
{
...
TEST (ModelStateTest, FrameTest)
{
  ...
  ros::NodeHandle nh;
  ros::Publisher modelStatePub = nh.advertise<gazebo_msgs_new::ModelState>("/gazebo/set_model_state", 5);
  ros::Subscriber modelStatesSub = nh.subscribe("/gazebo/model_states", 1, &modelStatesCallback);

  gazebo_msgs_new::ModelState state;
