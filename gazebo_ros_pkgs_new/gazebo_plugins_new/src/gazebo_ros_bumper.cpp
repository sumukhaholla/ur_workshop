...
void GazeboRosBumper::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  this->contact_pub_ = this->rosnode_->advertise<gazebo_msgs_new::ContactsState>(
...
/*
    ...
    this->frame_name_ != "/map" && this->frame_name_ != "map")
  {
    ...
    if (!myFrame)
    {
      ROS_INFO_NAMED("bumper", "gazebo_ros_new_bumper plugin: frameName: %s does not exist"
...
*/
  ...
  */
  ...
  for (unsigned int i = 0; i < contactsPacketSize; ++i)
  {
    ...
    gazebo_msgs_new::ContactState state;
    /// \TODO:
