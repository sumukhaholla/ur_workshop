...
void GazeboRosP3D::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  if (!this->link_)
  {
    ROS_FATAL_NAMED("p3d", "gazebo_ros_new_p3d plugin error: bodyName: %s does not exist\n",
      ...
      this->frame_name_ != "map")
  {
    ...
    {
      ROS_ERROR_NAMED("p3d", "gazebo_ros_new_p3d plugin: frameName: %s does not exist, will"
