...
void GazeboRosBlockLaser::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  ROS_DEBUG_NAMED("block_laser", "gazebo_ros_new_laser plugin should set minimum intensity to %f due to cutoff in hokuyo filters." , this->hokuyo_min_intensity_);
...
void GazeboRosBlockLaser::OnNewLaserScans()
{
  ...
  {
    ROS_INFO_NAMED("block_laser", "gazebo_ros_new_block_laser topic name not set");
  }
}
